...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 6143925A404
	for <lists+linux-kernel@lfdr.de>; Wed,  2 Sep 2020 05:23:37 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727080AbgIBDXQ (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Tue, 1 Sep 2020 23:23:16 -0400
Received: from mailout2.samsung.com ([203.254.224.25]:41367 "EHLO
        mailout2.samsung.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1726984AbgIBDXJ (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Tue, 1 Sep 2020 23:23:09 -0400
Received: from epcas1p2.samsung.com (unknown [182.195.41.46])
        by mailout2.samsung.com (KnoxPortal) with ESMTP id 20200902032303epoutp0209e4997f2f217493c589cd546ea77d21~w2XlzIw5y0739107391epoutp02A
        for <linux-kernel@vger.kernel.org>; Wed,  2 Sep 2020 03:23:03 +0000 (GMT)
DKIM-Filter: OpenDKIM Filter v2.11.0 mailout2.samsung.com 20200902032303epoutp0209e4997f2f217493c589cd546ea77d21~w2XlzIw5y0739107391epoutp02A
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=samsung.com;
        s=mail20170921; t=1599016983;
        bh=TI+cpN7/B8OfKg1nJFUS7iXmJroMWwBaVt2zeoMjIbY=;
        h=Subject:Reply-To:From:To:CC:In-Reply-To:Date:References:From;
        b=nqgI8N5j4vJAXQH9/+tYloUaibI4DrfTbbL53Cc4quGC25i5SiempKHfpaWC9iCU3
         c0JXjAdrCNB3foOxX2rzmam3uo8lKvXlPlnGRNhd5oegInJp73hNlNoOhNRobD0xrg
         nKHm0BqiK+F20VSSu3XlKEe2QtYbbEdG6mVNtcnU=
Received: from epcpadp2 (unknown [182.195.40.12]) by epcas1p3.samsung.com
        (KnoxPortal) with ESMTP id
        20200902032302epcas1p36dafb493d4198f7c42d72d238a0e172a~w2XlW40Ku2673226732epcas1p3z;
        Wed,  2 Sep 2020 03:23:02 +0000 (GMT)
Mime-Version: 1.0
Subject: [PATCH v11 3/4] scsi: ufs: L2P map management for HPB read
...
X-Msg-Type: PERSONAL
X-Reply-Demand: N
Message-ID: <1431530910.81599016982864.JavaMail.epsvc@epcpadp2>
Date:   Wed, 02 Sep 2020 12:21:51 +0900
X-CMS-MailID: 20200902032151epcms2p3d9dc9123d114fda69dbc48c73202d243
...
index 45eb80116bfd..2ac4cd6e4aa4 100644
...
@@ -35,8 +101,758 @@ void ufshpb_prep(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
 {
 }
...
@@ -47,6 +863,8 @@ static void ufshpb_init_subregion_tbl(struct ufshpb_lu *hpb,
 	for (srgn_idx = 0; srgn_idx < rgn->srgn_cnt; srgn_idx++) {
...
+
 		...
 		srgn->srgn_state = HPB_SRGN_UNUSED;
@@ -78,6 +896,8 @@ static void ufshpb_init_lu_parameter(struct ufs_hba *hba,
...
+		hpb_lu_info->max_active_rgns - hpb_lu_info->num_pinned;
 	...
 	rgn_mem_size = (1ULL << hpb_dev_info->rgn_size) * HPB_RGN_SIZE_UNIT
 			* HPB_ENTRY_SIZE;
@@ -135,6 +955,9 @@ static int ufshpb_alloc_region_tbl(struct ufs_hba *hba, struct ufshpb_lu *hpb)
...
+
 		if (rgn_idx == hpb->rgns_per_lu - 1)
 			srgn_cnt = ((hpb->srgns_per_lu - 1) %
 				    hpb->srgns_per_rgn) + 1;
@@ -144,7 +967,13 @@ static int ufshpb_alloc_region_tbl(struct ufs_hba *hba, struct ufshpb_lu *hpb)
...
+		}
 	...
 	return 0;
@@ -168,7 +997,10 @@ static void ufshpb_destroy_subregion_tbl(struct ufshpb_lu *hpb,
 		struct ufshpb_subregion *srgn;
...
@@ -246,12 +1078,46 @@ static int ufshpb_lu_hpb_init(struct ufs_hba *hba, struct ufshpb_lu *hpb,
 	int ret;
...
@@ -282,6 +1148,34 @@ static struct ufshpb_lu *ufshpb_alloc_hpb_lu(struct ufs_hba *hba, int lun,
 	return NULL;
 }
...
+
 ...
 {
 	int err = 0;
@@ -370,6 +1264,8 @@ void ufshpb_reset_host(struct ufs_hba *hba)
 		if (ufshpb_get_state(hpb) != HPB_PRESENT)
 			continue;
...
@@ -388,6 +1284,7 @@ void ufshpb_suspend(struct ufs_hba *hba)
 		if (ufshpb_get_state(hpb) != HPB_PRESENT)
 			continue;
...
@@ -407,6 +1304,8 @@ void ufshpb_resume(struct ufs_hba *hba)
...
@@ -474,18 +1373,39 @@ static int ufshpb_get_lu_info(struct ufs_hba *hba, int lun,
...
@@ -513,6 +1433,9 @@ void ufshpb_init_hpb_lu(struct ufs_hba *hba, struct scsi_device *sdev)
 	if (!hpb)
...
@@ -524,6 +1447,57 @@ void ufshpb_init_hpb_lu(struct ufs_hba *hba, struct scsi_device *sdev)
...
@@ -589,7 +1563,11 @@ void ufshpb_init(struct ufs_hba *hba)
...
@@ -605,8 +1583,13 @@ void ufshpb_destroy_lu(struct ufs_hba *hba, struct scsi_device *sdev)
...
+
 	...
 	kfree(hpb);
@@ -614,8 +1597,18 @@ void ufshpb_destroy_lu(struct ufs_hba *hba, struct scsi_device *sdev)
