...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 4F929203186
	for <lists+linux-kernel@lfdr.de>; Mon, 22 Jun 2020 10:09:32 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726515AbgFVII5 (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Mon, 22 Jun 2020 04:08:57 -0400
Received: from mail.kernel.org ([198.145.29.99]:42902 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726422AbgFVIIm (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Mon, 22 Jun 2020 04:08:42 -0400
Received: from localhost (83-86-89-107.cable.dynamic.v4.ziggo.nl [83.86.89.107])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id 4A90B207C3;
        Mon, 22 Jun 2020 08:08:29 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1592813311;
        bh=nb//Eq3Uz2t4wyb88yJQKVQW37vwqAr8guKDfUDaw0w=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
        b=PvkyNhjbLE4IGNhWffB13q7ZCqePK83Er1G4TuUAo2rZ+0laydRq/9oYi491Qd0wq
         lpa4tBM++y4q0V//uqrA2tDvinK4Bl35OuaA86DiGx6C9F+LO+hmoWkKxlOHLqkLIA
         y2Lk0KpQ5UOcytTc6/lMKQYWJoKzc15Jzi9DSOmo=
From:   Greg Kroah-Hartman <gregkh@linuxfoundation.org>
To:     linux-kernel@vger.kernel.org, akpm@linux-foundation.org,
        torvalds@linux-foundation.org, stable@vger.kernel.org
Cc:     lwn@lwn.net, jslaby@suse.cz,
        Greg Kroah-Hartman <gregkh@linuxfoundation.org>
Subject: Re: Linux 4.14.185
Date:   Mon, 22 Jun 2020 10:08:16 +0200
Message-Id: <1592813295985@kroah.com>
X-Mailer: git-send-email 2.27.0
In-Reply-To: <1592813295201255@kroah.com>
References: <1592813295201255@kroah.com>
MIME-Version: 1.0
...
@@ -717,9 +717,8 @@ static int mmc_sdio_init_card(struct mmc_host *host, u32 ocr,
...
+			}
 			...
 			if (!nskb) {
 				dev_err(&hif_dev->udev->dev,
@@ -636,9 +641,9 @@ static void ath9k_hif_usb_rx_stream(struct hif_device_usb *hif_dev,
...
+	struct sk_buff *skb = rx_buf->skb;
 	...
 	if (!skb)
@@ -678,14 +683,15 @@ static void ath9k_hif_usb_rx_cb(struct urb *urb)
 	return;
 free:
 	kfree_skb(skb);
+	kfree(rx_buf);
...
-		usb_get_intfdata(usb_ifnum_to_if(urb->dev, 0));
 	...
 	if (!skb)
@@ -743,6 +749,7 @@ static void ath9k_hif_usb_reg_in_cb(struct urb *urb)
 	return;
 free:
 	kfree_skb(skb);
+	kfree(rx_buf);
...
@@ -878,6 +895,8 @@ static int ath9k_hif_usb_alloc_rx_urbs(struct hif_device_usb *hif_dev)
 err_skb:
 	usb_free_urb(urb);
 err_urb:
+	kfree(rx_buf);
...
@@ -941,6 +970,8 @@ static int ath9k_hif_usb_alloc_reg_in_urbs(struct hif_device_usb *hif_dev)
 err_skb:
 	usb_free_urb(urb);
 err_urb:
+	kfree(rx_buf);
...
@@ -971,7 +1002,7 @@ static int ath9k_hif_usb_alloc_urbs(struct hif_device_usb *hif_dev)
 	return -ENOMEM;
 }
