...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id C1E5723D4F5
	for <lists+linux-kernel@lfdr.de>; Thu,  6 Aug 2020 03:07:42 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726829AbgHFBHf (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Wed, 5 Aug 2020 21:07:35 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:39612 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1726005AbgHFBHT (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Wed, 5 Aug 2020 21:07:19 -0400
Received: from mail-ej1-x630.google.com (mail-ej1-x630.google.com [IPv6:2a00:1450:4864:20::630])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id C9CF9C061574
        for <linux-kernel@vger.kernel.org>; Wed,  5 Aug 2020 18:07:17 -0700 (PDT)
Received: by mail-ej1-x630.google.com with SMTP id kq25so35487522ejb.3
        for <linux-kernel@vger.kernel.org>; Wed, 05 Aug 2020 18:07:17 -0700 (PDT)
...
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        h=x-gm-message-state:mime-version:from:date:message-id:subject:to:cc
         :content-transfer-encoding;
        bh=C1JOKfb/oSKNf+fN7PbukYf3xMkI9ndxH3d8+CVx60M=;
        b=
X-Gm-Message-State: AOAM5306AvrCDgyWJvuwXNQfKJby3DYsRelzKTzsM6jvJ6gqoBnScKU9
        bCaOSC08co/xbNIAjsn3Uj8KAjfPKVH5hbW3n1jDj3pZ
X-Google-Smtp-Source: ABdhPJw6L6CPaMkWkI2nFBD4hz44uX5FGFOToMCGpg/73KKggANcTedOtsyhOiuy2RWOsMsNjC/0Ar/me21M9wXGAM8=
X-Received: by 2002:a17:907:72c8:: with SMTP id du8mr1885182ejc.237.1596676035469;
...
Aditya Pakki (8):
      ...
      drm/nouveau/drm/noveau: fix reference count leak in nouveau_fbcon_ope=
n
...
ps
      ...
      drm/amdgpu: make sure to reserve tmr region on all asics which suppor=
t it
      drm/amdgpu: rework runtime pm enablement for BACO
      drm/amdgpu: enable runtime pm on vega10 when noretry=3D0
...
Aurabindo Pillai (6):
      ...
      drm/amd/display: Call dsc related functions indirectly via dc interfa=
ce
...
can bootstrap
      drm/nouveau/pmu/gm20x: don't pretend we support loading with our cust=
om FW
...
NVC57D_HEAD_SET_HEAD_USAGE_BOUNDS_UPSCALING_ALLOWED to TRUE
      ...
      drm/nouveau/nvif: give every device object a human-readable identifie=
r
      ...
      drm/nouveau/nvif: give every usermode object a human-readable identif=
ier
...
nvif_push interface
      ...
      drm/nouveau/kms/nv50-: convert core head_olut_set() to new push macro=
s
      drm/nouveau/kms/nv50-: convert core head_olut_clr() to new push macro=
s
      drm/nouveau/kms/nv50-: convert core head_core_set() to new push macro=
s
      drm/nouveau/kms/nv50-: convert core head_core_clr() to new push macro=
s
      drm/nouveau/kms/nv50-: convert core head_curs_set() to new push macro=
s
      drm/nouveau/kms/nv50-: convert core head_curs_clr() to new push macro=
s
      ...
      drm/nouveau/bo: split buffer move functions into their own source fil=
es
...
Chris Wilson (124):
      ...
      drm: Help unconfuse gcc, avoid accidental impossible unsigned compari=
sons
      drm: Describe dp_rev for drm_dp_set_phy_test_pattern
      drm: Match drm_dp_send_clear_payload_id_table definition to declarati=
on
...
e
      ...
      drm/i915/selftests: Check for an initial-breadcrumb in wait_for_submi=
t()
      ...
      drm/i915/gt: Stop cross-polluting PIN_GLOBAL with PIN_USER with no-pp=
gtt
...
error injection
      ...
      drm/i915/gt: Move hsw GT workarounds from init_clock_gating to workar=
ounds
      drm/i915/gt: Move ivb GT workarounds from init_clock_gating to workar=
ounds
      drm/i915/gt: Move vlv GT workarounds from init_clock_gating to workar=
ounds
      drm/i915/gt: Move snb GT workarounds from init_clock_gating to workar=
ounds
      drm/i915/gt: Move ilk GT workarounds from init_clock_gating to workar=
ounds
      drm/i915/gt: Move gen4 GT workarounds from init_clock_gating to
workarounds
      drm/i915: Leave vma intact as they are discarded
      drm/i915/execlists: Lift opportunistic process_csb to before engine l=
ock
      drm/i915/selftests: Trim execlists runtime
      drm/i915/gt: Flush gen3 relocs harder, again
      drm/i915/selftests: Disable preemptive heartbeats over preemption tes=
ts
      drm/i915/selftests: Dump engine state and trace upon hanging after re=
set
      ...
      drm/i915/selftests: Use friendly request names for live_timeslice_rew=
ind
...
queue depths
      ...
      drm/i915/gt: Replace manual kmap_atomic() with pin_map for renderstat=
e
      ...
      drm/i915/gem: Only revoke the GGTT mmappings on aperture detiling cha=
nges
...
text
      ...
      drm/amdgpu: ensure 0 is returned for success in jpeg_v2_5_wait_for_id=
le
      drm/radeon: fix array out-of-bounds read and write issues
      drm/amd/display: remove redundant initialization of variable result
      drm/amdgpu: fix spelling mistake "Falied" -> "Failed"
      drm/i915/selftest: fix an error return path where err is not being se=
t
...
Daniel Vetter (31):
      ...
      drm/shmem-helpers: Don't call get/put_pages on imported dma-buf in vm=
ap
...
https://git.kernel.org/.../chunkuang.hu/linux into drm-next
      Merge branch 'linux-5.9' of git://github.com/skeggsb/linux into drm-n=
ext
...
Dejin Zheng (2):
      ...
      console: newport_con: fix an issue about leak related system resource=
s
...
Dmitry Osipenko (14):
      ...
      gpu: host1x: debug: Fix multiple channels emitting messages simultane=
ously
      ...
      drm/tegra: plane: Support 180=C2=B0 rotation
...
Douglas Anderson (10):
      ...
      drm/bridge: ti-sn65dsi86: Don't compile GPIO bits if not CONFIG_OF_GP=
IO
      drm/bridge: ti-sn65dsi86: Don't use kernel-doc comment for local arra=
y
      drm/bridge: ti-sn65dsi86: Fix kernel-doc typo ln_polr =3D> ln_polrs
...
to late_init
      ...
      drm/amd/powerplay: maximize code sharing between .hw_fini and .suspen=
d
      drm/amd/powerplay: move amdgpu_irq_src to the smu structure allocatio=
n
...
current power source
      ...
      drm/amd/powerplay: move maximum sustainable clock retrieving to .hw_i=
nit
      ...
      drm/amd/powerplay: fix compile error with ARCH=3Darc
      ...
      drm/amd/powerplay: bypass wrapper on retrieving current clock frequen=
cy
      drm/amd/powerplay: unshare the code for retrieving current clock freq=
uency
      drm/amd/powerplay: drop unused code and wrapper around clock retrievi=
ng
      ...
      drm/amd/powerplay: put dpm frequency setting common code in smu_v11_0=
.c
      ...
      drm/amd/powerplay: update the common API for performance level settin=
g
...
tting
      ...
      drm/amd/powerplay: sort the call flow on temperature ranges retrievin=
g
      ...
      drm/amd/powerplay: common API for disabling all features with excepti=
on
...
Gustavo A. R. Silva (7):
      drm/radeon/dpm: Replace one-element array and use struct_size() helpe=
r
...
Harigovindan P (1):
      drm/bridge: ti-sn65dsi86: ensure bridge suspend happens during PM sle=
ep
...
Hawking Zhang (8):
      ...
      drm/amdgpu: add atomfirmware helper funciton to query reserved fb siz=
e
...
Hyun Kwon (3):
      dmaengine: xilinx: dpdma: Add the Xilinx DisplayPort DMA engine drive=
r
...
Imre Deak (20):
      ...
      drm/i915/icl: Disable DIP on MST ports with the transcoder clock stil=
l on
      drm/i915: Fix the i915_dsc_fec_support debugfs file for DP MST connec=
tors
      drm/i915/dp_mst: Fix disabling MST on a port
      drm/dp_mst: Sanitize mgr->qlock locking in drm_dp_mst_wait_tx_reply()
      drm/i915/dp_mst: Work around out-of-spec adapters filtering short pul=
ses
      ...
      drm/i915/tgl+: Use the correct DP_TP_* register instances in MST enco=
ders
      drm/i915/dp_mst: Disable link training fallback on MST links
      drm/i915/dp_mst: Move clearing the ACT sent flag closer to its pollin=
g
      ...
      drm/i915/dp_mst: Enable VC payload allocation after transcoder is ena=
bled
      ...
      drm/i915: Fix the old vs. new epoch counter check during hotplug dete=
ct
...
Jack Xiao (27):
      ...
      drm/amdgpu/mes10.1: add the helper function for mes command submissio=
n
...
Jonathan Marek (16):
      drm/msm: fix unbalanced pm_runtime_enable in adreno_gpu_{init, cleanu=
p}
...
Kenneth Feng (10):
      ...
      drm/amd/powerplay: show gfxclk=3D0 in gfxoff state
...
Kevin Wang (5):
      drm/amd/powerplay: remove the support of xgmi pstate on vega20 from s=
wsmu
...
()
      ...
      drm: bridge: dw-hdmi: Pass drm_connector to internal functions as nee=
ded
      ...
      drm: panel: simple: Warn in case of incorrect bus format for LVDS pan=
els
...
sienna_cichlid (v2)
      ...
      drm/amdgpu/powerplay: add initial swSMU support for sienna_cichlid (v=
2)
      drm/amd/powerplay: add support to set performance level for sienna_ci=
chlid
      ...
      drm/amd/powerplay: enable Graphics Clock Deep Sleep for sienna_cichli=
d
      ...
      drm/amd/powerplay: support pcie value set and update for sienna_cichl=
id
      ...
      drm/amdgpu: skip for reroute ih for sienna_cichlid psp ring init curr=
ently
      ...
      drm/amdgpu: update the num of queue per pipe for mec on sienna_cichli=
d
...
sienna_cichlid.
      ...
      drm/amd/powerplay: support mclk socclk limit value set for sienna_cic=
hlid.
...
__dw_hdmi_probe() bailout path
      ...
      drm/imx: imx-ldb: Disable both channels for split mode in enc->disabl=
e()
...
Lyude Paul (18):
      ...
      drm/nouveau/kms/nv140-: Don't modify depth in state during atomic com=
mit
...
Video Framebuffer Compression
      ...
      drm/meson: overlay: setup overlay for Amlogic FBC Scatter Memory layo=
ut
...
DMUB load path
      ...
      drm/amd/display: Only actually breakpoint if DEBUG_KERNEL_DC is enabl=
ed
...
Ondrej Jirman (12):
      ...
      drm/panel: st7703: Move generic part of init sequence to enable callb=
ack
      ...
      dt-binding: display: Allow a single port node on rocktech, jh057n0090=
0
...
Sowjanya Komatineni (3):
      ...
      gpu: host1x: mipi: Use readl_relaxed_poll_timeout() in tegra_mipi_wai=
t()
...
Steve Longerbeam (3):
      ...
      gpu: ipu-v3: image-convert: Wait for all EOFs before completing a til=
e
...
CMA functions
      ...
      drm/hisilicon/kirin: Set GEM CMA functions with DRM_GEM_CMA_DRIVER_OP=
S
...
flag
      ...
      drm/ast: Pass struct ast_private instance to cursor init/fini functio=
ns
      ...
      drm/mgag200: Set/clear <syncrst> field in display enable/disable help=
ers
...
Ville Syrj=C3=A4l=C3=A4 (65):
      ...
      drm/i915: Configure overlay cc_out precision based on crtc gamma conf=
ig
...
the vrefresh
      ...
      drm/i915/fbc: Don't clear busy_bits for origin=3D=3DGTT
      ...
      drm/i915/fbc: Allow FBC to recompress after a 3D workload on i85x/i86=
5
      ...
      drm/edid: Allow looking for ext blocks starting from a specified inde=
x
...
specs update"
      drm/amd/display: allow query ddc data over aux to be read only operat=
ion
      drm/amd/display: DP link layer test 4.2.1.1 fix due to specs update
...
Yi-Ling Chen (1):
      drm/amd/display: Fixed using wrong eDP power sequence function pointe=
r
...
shaoyunl (4):
 ...
 .../inc/dmub_cmd_dal.h =3D> amdgpu/sdma_common.h}    |    42 +-
 ...
 .../inc/dmub_types.h =3D> dc/dce/dmub_hw_lock_mgr.c} |    78 +-
...
++++
 ...
 drivers/gpu/drm/ast/{ast_ttm.c =3D> ast_mm.c}        |    79 +-
 ...
 .../ingenic/{ingenic-drm.c =3D> ingenic-drm-drv.c}   |   685 +-
 ...
 .../nouveau/nvkm/subdev/pmu/{gp100.c =3D> gm200.c}   |    18 +-
 ...
 delete mode 100644 Documentation/devicetree/bindings/display/brcm,bcm-vc4.=
txt
 create mode 100644
...
Documentation/devicetree/bindings/display/connector/analog-tv-connector.txt
 create mode 100644
Documentation/devicetree/bindings/display/connector/analog-tv-connector.yam=
l
 delete mode 100644
...
Documentation/devicetree/bindings/display/connector/vga-connector.yaml
 create mode 100644 Documentation/devicetree/bindings/display/ingenic,ipu.y=
aml
 delete mode 100644 Documentation/devicetree/bindings/display/ingenic,lcd.t=
xt
 create mode 100644 Documentation/devicetree/bindings/display/ingenic,lcd.y=
aml
 delete mode 100644
...
Documentation/devicetree/bindings/dma/xilinx/xlnx,zynqmp-dpdma.yaml
 ...
 rename drivers/gpu/drm/amd/{display/dmub/inc/dmub_cmd_dal.h =3D>
amdgpu/sdma_common.h} (62%)
 ...
 create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dcn30/dcn30_clk_=
mgr.c
 create mode 100644 drivers/gpu/drm/amd/display/dc/clk_mgr/dcn30/dcn30_clk_=
mgr.h
 create mode 100644
...
drivers/gpu/drm/amd/display/dc/clk_mgr/dcn30/dcn30_clk_mgr_smu_msg.h
 rename drivers/gpu/drm/amd/display/{dmub/inc/dmub_types.h =3D>
...
drivers/gpu/drm/amd/display/dc/dml/dcn30/display_rq_dlg_calc_30.h
 create mode 100644 drivers/gpu/drm/amd/display/dc/gpio/dcn30/hw_factory_dc=
n30.c
 create mode 100644 drivers/gpu/drm/amd/display/dc/gpio/dcn30/hw_factory_dc=
n30.h
 create mode 100644
...
drivers/gpu/drm/amd/display/dc/gpio/dcn30/hw_translate_dcn30.h
 create mode 100644 drivers/gpu/drm/amd/display/dc/irq/dcn30/irq_service_dc=
n30.c
 create mode 100644 drivers/gpu/drm/amd/display/dc/irq/dcn30/irq_service_dc=
n30.h
...
drivers/gpu/drm/amd/include/asic_reg/athub/athub_2_1_0_sh_mask.h
 create mode 100755 drivers/gpu/drm/amd/include/asic_reg/dcn/dcn_3_0_0_offs=
et.h
 create mode 100755 drivers/gpu/drm/amd/include/asic_reg/dcn/dcn_3_0_0_sh_m=
ask.h
 create mode 100755 drivers/gpu/drm/amd/include/asic_reg/dcn/dpcs_3_0_0_off=
set.h
 create mode 100755
drivers/gpu/drm/amd/include/asic_reg/dcn/dpcs_3_0_0_sh_mask.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/gc/gc_10_3_0_defau=
lt.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/gc/gc_10_3_0_offse=
t.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/gc/gc_10_3_0_sh_ma=
sk.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/uvd/uvd_3_1_d.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/uvd/uvd_3_1_sh_mas=
k.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/vcn/vcn_3_0_0_offs=
et.h
 create mode 100644 drivers/gpu/drm/amd/include/asic_reg/vcn/vcn_3_0_0_sh_m=
ask.h
 create mode 100644
...
drivers/gpu/drm/amd/powerplay/inc/smu11_driver_if_sienna_cichlid.h
 ...
 rename drivers/gpu/drm/amd/{display/dmub/inc/dmub_cmd_vbios.h =3D>
powerplay/sienna_cichlid_ppt.h} (74%)
 ...
 rename drivers/gpu/drm/ast/{ast_ttm.c =3D> ast_mm.c} (61%)
 ...
 rename drivers/gpu/drm/ingenic/{ingenic-drm.c =3D> ingenic-drm-drv.c} (53%=
)
 ...
 rename drivers/gpu/drm/nouveau/nvkm/subdev/pmu/{gp100.c =3D> gm200.c} (79%=
)
