...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 85C6A209FFE
	for <lists+linux-kernel@lfdr.de>; Thu, 25 Jun 2020 15:33:36 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2405133AbgFYNc6 (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Thu, 25 Jun 2020 09:32:58 -0400
Received: from mail.kernel.org ([198.145.29.99]:50894 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2404872AbgFYNcy (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Thu, 25 Jun 2020 09:32:54 -0400
Received: from localhost (83-86-89-107.cable.dynamic.v4.ziggo.nl [83.86.89.107])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id 68CD420781;
        Thu, 25 Jun 2020 13:32:36 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1593091959;
        bh=3/641GndvQe7+DlS2MYyjU7Y4tlW5sCcDoMj4q/73/s=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
        b=TbFSo7iyHkga31cGncsS5Vij0sx+y0f5rmMdVJ53jg5nPwOV8XJ8ivaP1zSj64efP
         c78ZrnkPdfabomuE+OklpnfOWO7f6wE1TrepVRS0H8RGbPqG7o/kWk316YjDUYytQ/
         OIhef16uQTyD4NDdVSBb3VCwcqiCDLGKWa66XHQ8=
From:   Greg Kroah-Hartman <gregkh@linuxfoundation.org>
To:     linux-kernel@vger.kernel.org, akpm@linux-foundation.org,
        torvalds@linux-foundation.org, stable@vger.kernel.org
Cc:     lwn@lwn.net, jslaby@suse.cz,
        Greg Kroah-Hartman <gregkh@linuxfoundation.org>
Subject: Re: Linux 5.4.49
Date:   Thu, 25 Jun 2020 15:32:26 +0200
Message-Id: <159309194546188@kroah.com>
X-Mailer: git-send-email 2.27.0
In-Reply-To: <159309194526196@kroah.com>
References: <159309194526196@kroah.com>
MIME-Version: 1.0
...
index 3f78373f708a..05d1657170b4 100644
...
index 5f350cc71a2f..c3668187b844 100644
...
index 8856ce476ccf..082178a0f41a 100644
...
+		.init_count = ARRAY_SIZE(meson8m2_gp_pll_init_regs),
 	...
 	.hw.init = &(struct clk_init_data){
 		.name = "gp_pll_dco",
@@ -3491,54 +3500,87 @@ static struct clk_regmap *const meson8b_clk_regmaps[] = {
...
@@ -3547,22 +3589,22 @@ static int meson8b_clk_reset_update(struct reset_controller_dev *rcdev,
 {
...
index 2028dc017f7a..47e7d11ca0c9 100644
...
@@ -2230,6 +2226,12 @@ void dc_commit_updates_for_stream(struct dc *dc,
...
index 4b7aaad07423..006d6087700f 100644
...
@@ -3507,33 +3519,29 @@ static int drm_dp_dpcd_write_payload(struct drm_dp_mst_topology_mgr *mgr,
  */
...
+	pr_cont("\n");
 ...
 #else /* ifdef DEBUG */
@@ -706,11 +705,9 @@ static inline void i2c_pxa_stop_message(struct pxa_i2c *i2c)
 {
 	u32 icr;
...
index 8d0f15f27dc5..0a95afaa48fe 100644
...
@@ -706,7 +708,7 @@ static int bmp180_measure(struct bmp280_data *data, u8 ctrl_meas)
 	unsigned int ctrl;
...
+		reinit_completion(&data->done);
 	...
 	if (ret)
@@ -962,6 +964,9 @@ static int bmp085_fetch_eoc_irq(struct device *dev,
...
index 4540b00ccee9..0502c90c83ed 100644
...
@@ -4564,7 +4556,7 @@ static int hns_roce_v2_query_qp(struct ib_qp *ibqp, struct ib_qp_attr *qp_attr,
...
index d609f4659afb..fd75a9043bf1 100644
...
index 842f2210dc7e..3a5d2890fe2a 100644
...
@@ -1434,8 +1441,10 @@ static int fastrpc_rpmsg_probe(struct rpmsg_device *rpdev)
 					    domains[domain_id]);
...
index e9b7c2dfc730..5ce4766a6c9e 100644
...
@@ -556,7 +559,7 @@ static void perf_free_inbuf(struct perf_peer *peer)
 		return;
...
@@ -585,8 +588,9 @@ static int perf_setup_inbuf(struct perf_peer *peer)
...
+					 GFP_KERNEL);
 	if (!peer->inbuf) {
 		dev_err(&perf->ntb->dev, "Failed to alloc inbuf of %pa\n",
 			&peer->inbuf_size);
@@ -636,6 +640,7 @@ static void perf_service_work(struct work_struct *work)
...
@@ -652,7 +657,7 @@ static int perf_init_service(struct perf_ctx *perf)
 {
 	u64 mask;
...
@@ -1051,8 +1056,9 @@ static int perf_submit_test(struct perf_peer *peer)
...
+		return ret;
 	...
 	if (test_and_set_bit_lock(0, &perf->busy_flag))
 		return -EBUSY;
@@ -1418,10 +1424,21 @@ static int perf_init_peers(struct perf_ctx *perf)
 			peer->gidx = pidx;
 		}
...
+
 	for (pidx = 0; pidx < perf->pcnt; pidx++) {
 		...
 		if (ret)
@@ -1517,4 +1534,3 @@ static void __exit perf_exit(void)
...
index 9425354aef99..a497e7c1f4fc 100644
...
index 36a3eb4ad4c5..75408b9f232d 100644
...
+		gsm_data_kick(gsm, NULL);
 		...
 		break;
 	case CMD_FCOFF:
@@ -2525,7 +2529,7 @@ static void gsmld_write_wakeup(struct tty_struct *tty)
 	/* Queue poll */
...
index c30c5b1c478c..17340864a540 100644
...
@@ -1505,6 +1513,10 @@ static void dwc3_gadget_ep_skip_trbs(struct dwc3_ep *dep, struct dwc3_request *r
 {
 	int i;
...
@@ -1595,6 +1607,8 @@ int __dwc3_gadget_ep_set_halt(struct dwc3_ep *dep, int value, int protocol)
 {
 	struct dwc3_gadget_ep_cmd_params	params;
...
@@ -1631,13 +1645,37 @@ int __dwc3_gadget_ep_set_halt(struct dwc3_ep *dep, int value, int protocol)
 		else
...
index 769749ca5961..e9a49007cce4 100644
...
+
 	...
 	if (ret)
 		return ret;
@@ -331,6 +454,14 @@ static int ehci_platform_resume(struct device *dev)
 	}
...
index f0891bd8444c..d6359c37c9e5 100644
...
@@ -1726,8 +1731,11 @@ void vfio_config_free(struct vfio_pci_device *vdev)
 	vdev->vconfig = NULL;
...
index 3dd37a998ea9..66214707a945 100644
--- a/fs/fuse/file.c
+++ b/fs/fuse/file.c
...
index e7e483cdbea6..a677aa84ccb6 100644
...
@@ -1259,21 +1262,10 @@ static inline __u16 t_error(const struct trace_entry *ent)
...
@@ -1399,13 +1391,13 @@ static void blk_log_with_error(struct trace_seq *s,
...
index 8552874e5aac..204d87e7c9b1 100644
--- a/net/core/dev.c
+++ b/net/core/dev.c
@@ -79,6 +79,7 @@
 #include <linux/sched.h>
...
index 8291568b707f..6bbc118bf00e 100644
...
@@ -861,6 +865,7 @@ static void sock_hash_free(struct bpf_map *map)
 {
...
+	struct hlist_head unlink_list;
 	...
 	int i;
@@ -872,13 +877,32 @@ static void sock_hash_free(struct bpf_map *map)
 	synchronize_rcu();
...
index e62e1d7815aa..f7964d1ec486 100644
...
index ad8f38380aa3..6c391e5fad2a 100644
--- a/sound/usb/pcm.c
+++ b/sound/usb/pcm.c
...
+
 	...
 	return 0;
@@ -1575,6 +1580,8 @@ static void prepare_playback_urb(struct snd_usb_substream *subs,
