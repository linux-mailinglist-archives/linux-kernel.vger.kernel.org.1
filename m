...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 8CA2F1F228E
	for <lists+linux-kernel@lfdr.de>; Tue,  9 Jun 2020 01:10:28 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728251AbgFHXIq (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Mon, 8 Jun 2020 19:08:46 -0400
Received: from mx0a-00082601.pphosted.com ([67.231.145.42]:7544 "EHLO
        mx0a-00082601.pphosted.com" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S1727990AbgFHXHt (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Mon, 8 Jun 2020 19:07:49 -0400
Received: from pps.filterd (m0109333.ppops.net [127.0.0.1])
        by mx0a-00082601.pphosted.com (8.16.0.42/8.16.0.42) with SMTP id 058N0rbh022442
        for <linux-kernel@vger.kernel.org>; Mon, 8 Jun 2020 16:07:46 -0700
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=fb.com; h=from : to : cc : subject
 : date : message-id : in-reply-to : references : mime-version :
 content-transfer-encoding : content-type; s=facebook;
 bh=Ne3L7TbRwa9jGrk6MAgjBVaUYJ5CM2+49sFWuuAhgX8=;
 b=Znx63QNOMHA9eAKbsK5DooliSHeLQADW3hvZvjwXAz9HJUXcSNWNNhuTd0Cz9zRfYldR
 hHwGABg4k64mSP4OhkyDrqda2cUMF20nAWwtNeplorq8DoA4JivwDb6ocgovOuxqlx3l
 uWK5QfpjGKS38Y1dsQ9WvbS1dta37xMXuho= 
Received: from mail.thefacebook.com ([163.114.132.120])
        by mx0a-00082601.pphosted.com with ESMTP id 31gtucqnb6-10
        (version=TLSv1.2 cipher=ECDHE-RSA-AES128-GCM-SHA256 bits=128 verify=NOT)
        for <linux-kernel@vger.kernel.org>; Mon, 08 Jun 2020 16:07:46 -0700
Received: from intmgw001.06.prn3.facebook.com (2620:10d:c085:208::11) by
 mail.thefacebook.com (2620:10d:c085:11d::6) with Microsoft SMTP Server
 (version=TLS1_2, cipher=TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256) id
 15.1.1979.3; Mon, 8 Jun 2020 16:07:09 -0700
Received: by devvm1291.vll0.facebook.com (Postfix, from userid 111017)
        id 7A8B71D8FE5C; Mon,  8 Jun 2020 16:07:00 -0700 (PDT)
...
Subject: [PATCH v6 17/19] mm: memcg/slab: use a single set of kmem_caches for all allocations
Date:   Mon, 8 Jun 2020 16:06:52 -0700
Message-ID: <20200608230654.828134-18-guro@fb.com>
...
@@ -3788,8 +3784,8 @@ static int setup_kmem_cache_nodes(struct kmem_cache=
 *cachep, gfp_t gfp)
 }
=20
...
+			    int batchcount, int shared, gfp_t gfp)
 {
 	...
 	int cpu;
@@ -3834,30 +3830,6 @@ static int __do_tune_cpucache(struct kmem_cache *c=
achep, int limit,
...
-
 ...
 {
@@ -3870,13 +3842,6 @@ static int enable_cpucache(struct kmem_cache *cach=
ep, gfp_t gfp)
 	if (err)
 		goto end;
=20
...
-
 ...
 {
 	/*
@@ -316,38 +256,25 @@ static inline size_t obj_full_size(struct kmem_cach=
e *s)
...
@@ -366,15 +293,27 @@ static inline void mod_objcg_state(struct obj_cgrou=
p *objcg,
=20
...
@@ -393,13 +332,19 @@ static inline void memcg_slab_free_hook(struct kmem=
...
+
 	...
 	mod_objcg_state(objcg, page_pgdat(page), cache_vmstat_idx(s),
 			-obj_full_size(s));
@@ -407,35 +352,7 @@ static inline void memcg_slab_free_hook(struct kmem_=
...
-
 ...
 {
 	return false;
@@ -456,16 +373,17 @@ static inline void memcg_free_page_obj_cgroups(stru=
ct page *page)
 {
 }
=20
...
=20
@@ -473,11 +391,6 @@ static inline void memcg_slab_free_hook(struct kmem_=
cache *s, struct page *page,
 					void *p)
 {
 }
...
@@ -491,27 +404,18 @@ static inline struct kmem_cache *virt_to_cache(cons=
t void *obj)
...
=20
 	mod_node_page_state(page_pgdat(page), cache_vmstat_idx(s),
@@ -522,20 +426,12 @@ static inline struct kmem_cache *cache_from_obj(str=
...
+	WARN_ONCE(cachep && cachep !=3D s,
 	...
 	return cachep;
@@ -587,7 +483,7 @@ static inline struct kmem_cache *slab_pre_alloc_hook(=
struct kmem_cache *s,
=20
 	if (memcg_kmem_enabled() &&
...
@@ -606,8 +502,8 @@ static inline void slab_post_alloc_hook(struct kmem_c=
ache *s,
 					 s->flags, flags);
 	}
=20
...
@@ -1540,10 +1536,8 @@ static inline struct page *alloc_slab_page(struct =
kmem_cache *s,
 	else
...
=20
@@ -4290,7 +4283,6 @@ static struct kmem_cache * __init bootstrap(struct =
...
=20
 	...
 	if (s) {
@@ -4359,12 +4351,6 @@ __kmem_cache_alias(const char *name, unsigned int =
...
-
 		if (sysfs_slab_alias(s, name)) {
 			s->refcount--;
 			s =3D NULL;
@@ -4386,7 +4372,6 @@ int __kmem_cache_create(struct kmem_cache *s, slab_=
flags_t flags)
 	if (slab_state <=3D UP)
 		return 0;
=20
...
+		kmem_cache_shrink(s);
 		...
 		return -EINVAL;
 	return length;
@@ -5590,99 +5575,9 @@ static ssize_t slab_attr_store(struct kobject *kob=
j,
 		return -EIO;
=20
