...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id B859C27F765
	for <lists+linux-kernel@lfdr.de>; Thu,  1 Oct 2020 03:29:24 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1730866AbgJAB3Q (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Wed, 30 Sep 2020 21:29:16 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:50840 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1730556AbgJAB25 (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Wed, 30 Sep 2020 21:28:57 -0400
Received: from mail-vs1-xe64.google.com (mail-vs1-xe64.google.com [IPv6:2607:f8b0:4864:20::e64])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id A7F4BC0613D0
        for <linux-kernel@vger.kernel.org>; Wed, 30 Sep 2020 18:28:56 -0700 (PDT)
Received: by mail-vs1-xe64.google.com with SMTP id e2so1857297vsr.7
        for <linux-kernel@vger.kernel.org>; Wed, 30 Sep 2020 18:28:56 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=broadcom.com; s=google;
        h=from:to:cc:subject:date:message-id:in-reply-to:references;
        bh=PL+bhr+Hh+YMYtBMX9/TEt0nGtGu7MImw2JiEnYcFrc=;
        b=EohIu1fRIzx81MMyOtdsvUjW6zpDlfOvyN28texHYt+TG14520eRLtTp0oukSiARbr
         2mFvuN3XVpU2LJBRM0jDbZo1yztbSHZsgL0yyfXM5rcAa070q3Fh6qsGqtstYwBDgS6E
         2dBAJ8T4IrhPU0Y3mpvERliER2K51w+Z1zl3w=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=1e100.net; s=20161025;
        h=x-gm-message-state:from:to:cc:subject:date:message-id:in-reply-to
         :references;
        bh=PL+bhr+Hh+YMYtBMX9/TEt0nGtGu7MImw2JiEnYcFrc=;
        b=
X-Gm-Message-State: AOAM5337cKtrIlpynDyqh0DNBV7pqq4DwAMZ1tmVfq4pyWwkc/dDY4/Y
        SI1ayuEoJ5Lr9Wdxjj6UXYy4ADhykymnM0ECid6IiYLQB8KA
X-Google-Smtp-Source: ABdhPJwhJv5gF0T30EhbW9pnkGnic+Izc8L93zc0qJkFGBPbmf4cqKG1EjaISSK56LFT0VprLiyqpFvGWt21
X-Received: by 2002:a67:e981:: with SMTP id b1mr3657322vso.43.1601515735690;
        Wed, 30 Sep 2020 18:28:55 -0700 (PDT)
Received: from lbrmn-lnxub113.broadcom.net ([192.19.228.250])
        by smtp-relay.gmail.com with ESMTPS id r25sm520472vkq.14.2020.09.30.18.28.53
...
Subject: [PATCH v5 09/15] misc: bcm-vk: add VK messaging support
Date:   Wed, 30 Sep 2020 18:28:04 -0700
Message-Id: <20201001012810.4172-10-scott.branden@broadcom.com>
...
---
 ...
 7 files changed, 2035 insertions(+), 3 deletions(-)
...
index 452333a32604..38b2bd575b97 100644
...
@@ -352,8 +452,28 @@ static inline bool bcm_vk_msgq_marker_valid(struct bcm_vk *vk)
 }
...
index a8bbc58bef9f..c23bfaa71a31 100644
...
@@ -908,11 +1159,35 @@ static int bcm_vk_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
 		}
 	}
...
+		goto err_irq;
 	...
 	vk->devid = id;
@@ -942,6 +1217,12 @@ static int bcm_vk_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
...
+
 	...
 	return 0;
@@ -991,6 +1275,13 @@ static int bcm_vk_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
 err_ida_remove:
...
+
 err_iounmap:
 	for (i = 0; i < MAX_BAR; i++) {
 		if (vk->bar[i])
@@ -1029,6 +1320,8 @@ static void bcm_vk_remove(struct pci_dev *pdev)
...
@@ -1052,6 +1345,11 @@ static void bcm_vk_remove(struct pci_dev *pdev)
