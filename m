...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 121152B3DB6
	for <lists+linux-kernel@lfdr.de>; Mon, 16 Nov 2020 08:33:27 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727743AbgKPHcU (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Mon, 16 Nov 2020 02:32:20 -0500
Received: from mxout70.expurgate.net ([91.198.224.70]:16796 "EHLO
        mxout70.expurgate.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1726247AbgKPHcU (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Mon, 16 Nov 2020 02:32:20 -0500
Received: from [127.0.0.1] (helo=localhost)
        by relay.expurgate.net with smtp (Exim 4.92)
        (envelope-from <ms@dev.tdt.de>)
        id 1keYzm-000If8-Mh; Mon, 16 Nov 2020 08:32:14 +0100
Received: from [195.243.126.94] (helo=securemail.tdt.de)
        by relay.expurgate.net with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
        (Exim 4.92)
        (envelope-from <ms@dev.tdt.de>)
        id 1keYzl-000T7q-Vh; Mon, 16 Nov 2020 08:32:14 +0100
Received: from securemail.tdt.de (localhost [127.0.0.1])
        by securemail.tdt.de (Postfix) with ESMTP id 9B8F9240049;
        Mon, 16 Nov 2020 08:32:13 +0100 (CET)
Received: from mail.dev.tdt.de (unknown [10.2.4.42])
        by securemail.tdt.de (Postfix) with ESMTP id 1D623240047;
        Mon, 16 Nov 2020 08:32:13 +0100 (CET)
Received: from mschiller01.dev.tdt.de (unknown [10.2.3.20])
        by mail.dev.tdt.de (Postfix) with ESMTPSA id 6D07020115;
        Mon, 16 Nov 2020 08:32:12 +0100 (CET)
...
Date:   Mon, 16 Nov 2020 08:31:45 +0100
Message-ID: <20201116073149.23219-2-ms@dev.tdt.de>
...
X-purgate: clean
X-purgate-ID: 151534::1605511934-0001632B-B9FFA228/0/0
...
@@ -164,6 +167,7 @@ struct x25_sock {
 	...
 	struct x25_facilities	facilities;
+	unsigned int		socket_defined_facilities;	/* set, if facilities changed =
by SIOCX25SFACILITIES */
 	struct x25_dte_facilities dte_facilities;
 	struct x25_calluserdata	calluserdata;
 	unsigned long 		vc_facil_mask;	/* inc_call facilities mask */
@@ -215,7 +219,8 @@ int x25_create_facilities(unsigned char *, struct x25=
_facilities *,
...
+ */
+struct x25_subscrip_struct {
+	char device[200-((2 * sizeof(unsigned long)) + sizeof(struct x25_facili=
ties) + (2 * sizeof(unsigned int)))];
+	unsigned int		dce;
+	unsigned int		lc;
+	struct x25_facilities	facilities;
+	unsigned long		t20;
...
+	char device[200-((2 * sizeof(compat_ulong_t)) + sizeof(struct compat_x2=
5_facilities) + (2 * sizeof(compat_uint_t)))];
+	compat_uint_t		dce;
+	compat_uint_t		lc;
...
@@ -366,13 +377,26 @@ static unsigned int x25_new_lci(struct x25_neigh *n=
b)
...
=20
 	return lci;
@@ -806,6 +830,10 @@ static int x25_connect(struct socket *sock, struct s=
ockaddr *uaddr,
 	if (!x25->neighbour)
 		goto out_put_route;
=20
...
+	len =3D x25_negotiate_facilities(skb, sk, &facilities, &dte_facilities,=
 nb);
 	if (len =3D=3D -1)
 		goto out_sock_put;
=20
@@ -1454,10 +1482,15 @@ static int x25_ioctl(struct socket *sock, unsigne=
...
@@ -1471,16 +1504,16 @@ static int x25_ioctl(struct socket *sock, unsigne=
d int cmd, unsigned long arg)
 		lock_sock(sk);
...
@@ -1488,19 +1521,20 @@ static int x25_ioctl(struct socket *sock, unsigne=
d int cmd, unsigned long arg)
...
+
 	...
 	if (copy_from_user(&x25_subscr, x25_subscr32, sizeof(*x25_subscr32)))
 		goto out;
@@ -1665,28 +1702,75 @@ static int compat_x25_subscr_ioctl(unsigned int c=
md,
 	if (nb =3D=3D NULL)
 		goto out_dev_put;
=20
...
+	nb->facilities.throughput  =3D 0;	/* by default don't negotiate
+						   throughput */
+	nb->facilities.reverse     =3D X25_DEFAULT_REVERSE;
+	nb->t20                    =3D sysctl_x25_restart_request_timeout;
 	/*
 	 * Enables negotiation
 	 */
...
=20
 	write_lock_bh(&x25_neigh_list_lock);
@@ -395,28 +420,75 @@ int x25_subscr_ioctl(unsigned int cmd, void __user =
*arg)
