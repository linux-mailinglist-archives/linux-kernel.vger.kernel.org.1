...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 24A1F2AFF3C
	for <lists+linux-kernel@lfdr.de>; Thu, 12 Nov 2020 06:45:48 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728031AbgKLFdE convert rfc822-to-8bit (ORCPT
        <rfc822;lists+linux-kernel@lfdr.de>); Thu, 12 Nov 2020 00:33:04 -0500
Received: from youngberry.canonical.com ([91.189.89.112]:42176 "EHLO
        youngberry.canonical.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1729000AbgKLBpc (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Wed, 11 Nov 2020 20:45:32 -0500
Received: from 1.general.jvosburgh.us.vpn ([10.172.68.206] helo=famine.localdomain)
        by youngberry.canonical.com with esmtpsa (TLS1.2:ECDHE_RSA_AES_128_GCM_SHA256:128)
        (Exim 4.86_2)
        (envelope-from <jay.vosburgh@canonical.com>)
        id 1kd1eN-00027e-Rv; Thu, 12 Nov 2020 01:43:48 +0000
Received: by famine.localdomain (Postfix, from userid 1000)
        id 591695FEE8; Wed, 11 Nov 2020 17:43:46 -0800 (PST)
Received: from famine (localhost [127.0.0.1])
        by famine.localdomain (Postfix) with ESMTP id 478C2A0409;
...
Subject: Re: [PATCH 3/5] bonding: rename slave to port where possible
In-reply-to: <20201106200436.943795-4-jarod@redhat.com>
References: <20201106200436.943795-1-jarod@redhat.com> <20201106200436.943795-4-jarod@redhat.com>
Comments: In-reply-to Jarod Wilson <jarod@redhat.com>
   message dated "Fri, 06 Nov 2020 15:04:34 -0500."
X-Mailer: MH-E 8.6+git; nmh 1.6; GNU Emacs 27.0.50
MIME-Version: 1.0
Content-Type: text/plain; charset="us-ascii"
Content-ID: <31303.1605145426.1@famine>
Content-Transfer-Encoding: 8BIT
Date:   Wed, 11 Nov 2020 17:43:46 -0800
Message-ID: <31311.1605145426@famine>
Precedence: bulk
...
> drivers/net/bonding/bond_main.c               | 2321 +++++++++--------
...
> include/net/lag.h                             |    2 +-
> 23 files changed, 2420 insertions(+), 2361 deletions(-)
>
>diff --git a/.clang-format b/.clang-format
>index 10dc5a9a61b3..037040f8662a 100644
>--- a/.clang-format
>+++ b/.clang-format
...
>index e06cf51f1773..97500ac1e7b6 100644
...
>index cd0fba6b0964..b21a208ea0d8 100644
...
> 			}
	...
	I believe that this block, and the later blocks that change
device drivers, should trigger inclusion the appropriate maintainers on
the Cc list.  This applies to patch #2 in the series as well, which I
didn't think of when reading it.
...
>index c3091e00dd5f..86608a87ed60 100644
...
> 	bool strict_match;
> };
>@@ -948,18 +948,18 @@ static int alb_upper_dev_walk(struct net_device *upper,
...
> 					vlan_dev_vlan_id(upper));
> 		}
>@@ -972,7 +972,7 @@ static int alb_upper_dev_walk(struct net_device *upper,
...
> 		kfree(tags);
> 	}
>@@ -980,21 +980,21 @@ static int alb_upper_dev_walk(struct net_device *upper,
> 	return 0;
> }
> 
>-static void alb_send_learning_packets(struct slave *slave, u8 mac_addr[],
...
> 	 * for that device.
>@@ -1004,43 +1004,43 @@ static void alb_send_learning_packets(struct slave *slave, u8 mac_addr[],
> 	rcu_read_unlock();
> }
> 
...
> }
> 
>@@ -1048,77 +1048,77 @@ static void alb_swap_mac_addr(struct slave *slave1, struct slave *slave2)
>  *
...
> 	}
> }
>@@ -1126,81 +1126,81 @@ static void alb_change_hw_addr_on_detach(struct bonding *bond, struct slave *sla
> /**
...
> 	}
> 
>@@ -1212,18 +1212,18 @@ static int alb_handle_addr_collision_on_attach(struct bonding *bond, struct slav
>  * @bond: bonding we're working on
...
>@@ -1232,16 +1232,16 @@ static int alb_set_mac_address(struct bonding *bond, void *addr)
> 	if (bond->alb_info.rlb_enabled)
> 		return 0;
> 
>-	bond_for_each_slave(bond, slave, iter) {
...
> 		if (res)
> 			goto unwind;
>@@ -1253,16 +1253,16 @@ static int alb_set_mac_address(struct bonding *bond, void *addr)
...
> 
> 	return res;
>@@ -1303,30 +1303,30 @@ void bond_alb_deinitialize(struct bonding *bond)
> }
> 
...
> 	}
> 
>@@ -1334,10 +1334,10 @@ static netdev_tx_t bond_do_alb_xmit(struct sk_buff *skb, struct bonding *bond,
...
> 	u32 hash_index;
> 
>@@ -1353,40 +1353,40 @@ struct slave *bond_xmit_tlb_slave_get(struct bonding *bond,
...
> 	struct ethhdr *eth_data;
>@@ -1483,7 +1483,7 @@ struct slave *bond_xmit_alb_slave_get(struct bonding *bond,
> 	case ETH_P_ARP:
> 		do_tx_balance = false;
...
> 	default:
> 		do_tx_balance = false;
>@@ -1493,33 +1493,33 @@ struct slave *bond_xmit_alb_slave_get(struct bonding *bond,
> 	if (do_tx_balance) {
> 		if (bond->params.tlb_dynamic_lb) {
...
>@@ -1528,9 +1528,9 @@ void bond_alb_monitor(struct work_struct *work)
> 					    alb_work.work);
> 	struct alb_bond_info *bond_info = &(BOND_ALB_INFO(bond));
> 	struct list_head *iter;
>-	struct slave *slave;
...
> 		goto re_arm;
>@@ -1545,15 +1545,15 @@ void bond_alb_monitor(struct work_struct *work)
...
> 		}
> 		bond_info->lp_counter = 0;
>@@ -1561,10 +1561,10 @@ void bond_alb_monitor(struct work_struct *work)
> 
...
>@@ -1587,10 +1587,10 @@ void bond_alb_monitor(struct work_struct *work)
...
> 
>@@ -1621,23 +1621,23 @@ void bond_alb_monitor(struct work_struct *work)
...
>@@ -1648,33 +1648,34 @@ int bond_alb_init_slave(struct bonding *bond, struct slave *slave)
> 	return 0;
> }
> 
...
>@@ -1689,28 +1690,28 @@ void bond_alb_handle_link_change(struct bonding *bond, struct slave *slave, char
> 	}
> 
...
>@@ -1719,57 +1720,57 @@ void bond_alb_handle_active_change(struct bonding *bond, struct slave *new_slave
...
> 	}
> }
>@@ -1779,8 +1780,8 @@ int bond_alb_set_mac_address(struct net_device *bond_dev, void *addr)
> {
> 	struct bonding *bond = netdev_priv(bond_dev);
...
>@@ -1792,28 +1793,28 @@ int bond_alb_set_mac_address(struct net_device *bond_dev, void *addr)
> 
...
>index e9cc7d68f3b9..96d26c4d4b03 100644
...
> 				     "0 for never (default), 1 for always.");
	...
	I don't agree that we should add new module parameters.  The
existing ones remain, so there is no incentive for users to change, and
portable code will end up with the old names anyway.  I'd prefer to the
only path to move is away from module parameters entirely, not just to
different names.

	I haven't seen actual use of the bonding module parameters in a
long time.  I'm willing to believe there are still users, but I haven't
seen examples of it in years.  If there is a use case that would
absolutely require module parameters (and could not migrate to a feature
complete netlink API) I'd like to hear about it.
...
>+/* Push the promiscuity flag down to appropriate ports */
	...
	This comment could instead be removed, it doesn't really add
anything.
...
> 
>@@ -1307,28 +1319,28 @@ static void bond_compute_features(struct bonding *bond)
...
>+	bond_dev->needed_headroom   = port_dev->needed_headroom;
...
> 		 struct netlink_ext_ack *extack)
	...
	The comment could just be deleted, and perhaps something like
"bond_add_port" better describes what the function does?  I presume this
choice is to match "release" on the other end, but "connect" makes me
think the bond itself is being connected to something, like connect(2).
...
>+	bond_queue_port_event(new_port);
	...
	Again, could remove the comment instead.
...
> 		return -EINVAL;
	...
	Another comment to delete instead.
...
> 
>@@ -2540,19 +2550,19 @@ static bool bond_has_this_ip(struct bonding *bond, __be32 ip)
...
>@@ -2571,8 +2581,8 @@ static void bond_arp_send(struct slave *slave, int arp_op, __be32 dest_ip,
> 			continue;
> 		}
> 
...
> 						tags->vlan_id);
> 		if (!skb) {
>@@ -2584,8 +2594,8 @@ static void bond_arp_send(struct slave *slave, int arp_op, __be32 dest_ip,
> 	}
> 	/* Set the outer tag */
...
> 				       outer_tag->vlan_id);
> 	}
>@@ -2634,7 +2644,7 @@ struct bond_vlan_tag *bond_verify_device_path(struct net_device *start_dev,
> 	return NULL;
> }
> 
...
> 	struct rtable *rt;
> 	struct bond_vlan_tag *tags;
>@@ -2642,8 +2652,8 @@ static void bond_arp_send_all(struct bonding *bond, struct slave *slave)
> 	int i;
> 
...
>@@ -2657,7 +2667,7 @@ static void bond_arp_send_all(struct bonding *bond, struct slave *slave)
...
> 			continue;
> 		}
>@@ -2674,8 +2684,8 @@ static void bond_arp_send_all(struct bonding *bond, struct slave *slave)
> 			goto found;
> 
> 		/* Not our device - skip */
...
> 		ip_rt_put(rt);
> 		continue;
>@@ -2683,45 +2693,45 @@ static void bond_arp_send_all(struct bonding *bond, struct slave *slave)
> found:
...
> 	} else if (!is_arp) {
> 		return RX_HANDLER_ANOTHER;
>@@ -2729,8 +2739,8 @@ int bond_arp_rcv(const struct sk_buff *skb, struct bonding *bond,
> 
...
>@@ -2754,47 +2764,47 @@ int bond_arp_rcv(const struct sk_buff *skb, struct bonding *bond,
...
>@@ -2816,7 +2826,7 @@ static bool bond_time_in_interval(struct bonding *bond, unsigned long last_act,
...
>@@ -2824,67 +2834,67 @@ static bool bond_time_in_interval(struct bonding *bond, unsigned long last_act,
>  */
...
> 			}
> 		}
>@@ -2893,32 +2903,32 @@ static void bond_loadbalance_arp_mon(struct bonding *bond)
...
> 		}
> 		rtnl_unlock();
>@@ -2930,9 +2940,9 @@ static void bond_loadbalance_arp_mon(struct bonding *bond)
...
>@@ -2941,60 +2951,60 @@ static int bond_ab_arp_inspect(struct bonding *bond)
> {
...
> 		}
> 	}
>@@ -3011,34 +3021,34 @@ static void bond_ab_arp_commit(struct bonding *bond)
> {
...
> 
> 			}
>@@ -3046,46 +3056,46 @@ static void bond_ab_arp_commit(struct bonding *bond)
> 			continue;
> 
> 		case BOND_LINK_DOWN:
>-			if (slave->link_failure_count < UINT_MAX)
...
> 	}
> 
>@@ -3098,79 +3108,79 @@ static void bond_ab_arp_commit(struct bonding *bond)
>  */
...
>+			    curr_active_port->dev->name);
	...
	Not one of my more eloquent log messages; as long as this is
being edited, I'd change the text to something along the lines of
"impossible: c_active %s c_arp %s both set"
...
> 		}
> 	}
>@@ -3185,7 +3195,7 @@ static void bond_activebackup_arp_mon(struct bonding *bond)
> 
...
> 
> 	rcu_read_lock();
>@@ -3223,8 +3233,8 @@ static void bond_activebackup_arp_mon(struct bonding *bond)
...
> 
> 		rtnl_unlock();
>@@ -3278,76 +3288,76 @@ static int bond_dev_netdev_event(unsigned long event,
> 	return NOTIFY_DONE;
> }
> 
...
> 		 * servitude.
> 		 */
>@@ -3358,21 +3368,21 @@ static int bond_slave_netdev_event(unsigned long event,
...
>@@ -3380,7 +3390,7 @@ static int bond_slave_netdev_event(unsigned long event,
> 		break;
...
> 	default:
> 		break;
>@@ -3393,7 +3403,7 @@ static int bond_slave_netdev_event(unsigned long event,
>  *
...
>@@ -3416,7 +3426,7 @@ static int bond_netdev_event(struct notifier_block *this,
> 	}
> 
...
> 	return NOTIFY_DONE;
> }
>@@ -3562,7 +3572,7 @@ void bond_work_init_all(struct bonding *bond)
...
>@@ -3572,25 +3582,25 @@ static void bond_work_cancel_all(struct bonding *bond)
...
> 		}
> 	}
>@@ -3621,7 +3631,7 @@ static int bond_open(struct net_device *bond_dev)
> 	}
> 
> 	if (bond_mode_can_use_xmit_hash(bond))
>-		bond_update_slave_arr(bond, NULL);
...
> 	return 0;
> }
>@@ -3715,7 +3725,7 @@ static void bond_get_stats(struct net_device *bond_dev,
> 	struct bonding *bond = netdev_priv(bond_dev);
...
> 
> 
>@@ -3727,14 +3737,14 @@ static void bond_get_stats(struct net_device *bond_dev,
...
>@@ -3745,7 +3755,7 @@ static void bond_get_stats(struct net_device *bond_dev,
...
>@@ -3799,7 +3809,7 @@ static int bond_do_ioctl(struct net_device *bond_dev, struct ifreq *ifr, int cmd
...
> 			return -EFAULT;
>@@ -3814,30 +3824,30 @@ static int bond_do_ioctl(struct net_device *bond_dev, struct ifreq *ifr, int cmd
...
> 		break;
> 	default:
>@@ -3864,19 +3874,19 @@ static void bond_set_rx_mode(struct net_device *bond_dev)
> {
> 	struct bonding *bond = netdev_priv(bond_dev);
> 	struct list_head *iter;
>-	struct slave *slave;
...
> 	}
> 	rcu_read_unlock();
>@@ -3885,17 +3895,17 @@ static void bond_set_rx_mode(struct net_device *bond_dev)
...
>@@ -3907,7 +3917,7 @@ static int bond_neigh_init(struct neighbour *n)
...
> 	if (ret)
> 		goto out;
>@@ -3920,8 +3930,8 @@ static int bond_neigh_init(struct neighbour *n)
> }
> 
...
>@@ -3937,33 +3947,33 @@ static int bond_neigh_setup(struct net_device *dev,
> 	return 0;
> }
> 
...
> 		}
> 	}
>@@ -3973,17 +3983,17 @@ static int bond_change_mtu(struct net_device *bond_dev, int new_mtu)
> 	return 0;
> 
> unwind:
>-	/* unwind from head to the slave that failed */
...
> 
> 	return res;
>@@ -3992,13 +4002,13 @@ static int bond_change_mtu(struct net_device *bond_dev, int new_mtu)
> /* Change HW address
>  *
...
>  */
	...
	This comment is really old and no longer accurate, e.g., setting
the bond down no longer releases all ports.  I'd suggest just removing
it entirely.
...
> 	struct list_head *iter;
> 	int res = 0;
>@@ -4019,19 +4029,18 @@ static int bond_set_mac_address(struct net_device *bond_dev, void *addr)
> 	if (!is_valid_ether_addr(ss->__data))
...
> 		}
> 	}
>@@ -4044,18 +4053,18 @@ static int bond_set_mac_address(struct net_device *bond_dev, void *addr)
...
> 	}
> 
>@@ -4063,84 +4072,84 @@ static int bond_set_mac_address(struct net_device *bond_dev, void *addr)
> }
> 
> /**
...
>@@ -4152,18 +4161,18 @@ static struct slave *bond_xmit_roundrobin_slave_get(struct bonding *bond,
> 
> 		iph = ip_hdr(skb);
...
> 	return NULL;
> }
>@@ -4172,135 +4181,135 @@ static netdev_tx_t bond_xmit_roundrobin(struct sk_buff *skb,
> 					struct net_device *bond_dev)
> {
> 	struct bonding *bond = netdev_priv(bond_dev);
>-	struct slave *slave;
...
> 	int agg_id = 0;
> 	int ret = 0;
>@@ -4309,11 +4318,11 @@ int bond_update_slave_arr(struct bonding *bond, struct slave *skipslave)
...
> 		goto out;
> 	}
>@@ -4325,79 +4334,79 @@ int bond_update_slave_arr(struct bonding *bond, struct slave *skipslave)
...
> }
>@@ -4407,13 +4416,13 @@ static netdev_tx_t bond_xmit_broadcast(struct sk_buff *skb,
> 				       struct net_device *bond_dev)
> {
> 	struct bonding *bond = netdev_priv(bond_dev);
>-	struct slave *slave = NULL;
...
> 
> 			if (!skb2) {
>@@ -4421,36 +4430,36 @@ static netdev_tx_t bond_xmit_broadcast(struct sk_buff *skb,
...
> 		}
> 	}
>@@ -4480,36 +4489,36 @@ static u16 bond_select_queue(struct net_device *dev, struct sk_buff *skb,
> 	return txq;
> }
> 
...
>@@ -4517,8 +4526,8 @@ static struct net_device *bond_xmit_get_slave(struct net_device *bond_dev,
> 		break;
> 	}
> 
>-	if (slave)
...
> }
> 
>@@ -4527,7 +4536,7 @@ static netdev_tx_t __bond_start_xmit(struct sk_buff *skb, struct net_device *dev
> 	struct bonding *bond = netdev_priv(dev);
> 
...
> 
> 	switch (BOND_MODE(bond)) {
>@@ -4564,7 +4573,7 @@ static netdev_tx_t bond_start_xmit(struct sk_buff *skb, struct net_device *dev)
...
>@@ -4573,12 +4582,12 @@ static netdev_tx_t bond_start_xmit(struct sk_buff *skb, struct net_device *dev)
> 	return ret;
> }
> 
...
> 	return speed;
> }
>@@ -4588,29 +4597,29 @@ static int bond_ethtool_get_link_ksettings(struct net_device *bond_dev,
> {
> 	struct bonding *bond = netdev_priv(bond_dev);
> 	struct list_head *iter;
>-	struct slave *slave;
...
>@@ -4653,11 +4662,11 @@ static const struct net_device_ops bond_netdev_ops = {
...
>@@ -4709,7 +4718,7 @@ void bond_setup(struct net_device *bond_dev)
...
> 	 */
> 
>@@ -4739,26 +4748,26 @@ void bond_setup(struct net_device *bond_dev)
> static void bond_uninit(struct net_device *bond_dev)
> {
> 	struct bonding *bond = netdev_priv(bond_dev);
>-	struct bond_up_slave *usable, *all;
...
> 	}
> 
>@@ -4897,10 +4906,10 @@ static int bond_check_params(struct bond_params *params)
...
>@@ -4909,11 +4918,11 @@ static int bond_check_params(struct bond_params *params)
...
>@@ -5125,23 +5134,23 @@ static int bond_check_params(struct bond_params *params)
...
>+/* end legacy sysfs interfaces */
	...
	I think the "legacy sysfs" description is confusing, as this
whole block is option names that don't appear in sysfs unless there's an
attr in the per_bond_attrs, and more than just these four have sysfs
attrs.  Perhaps organize the block into "in sysfs" and "not in sysfs"
sections for clarity, and call this set "backwards compatibility" names?
...
>+				"bonding_slave");
	...
	From the changelog at the top, I thought the sysfs API wasn't
going to change; is this an oversight?
...
>index e3a968e9e2a0..5be88bbdd96c 100644
...
>index 18db678ac85c..7a225ad52e72 100644
...
>@@ -1627,7 +1627,7 @@ enum netdev_priv_flags {
...
>  *
>@@ -2772,9 +2772,9 @@ extern rwlock_t				dev_base_lock;		/* Device list lock */
> 						     dev_list)
...
>@@ -4667,7 +4667,7 @@ struct sk_buff *skb_mac_gso_segment(struct sk_buff *skb,
...
> };
> 
>@@ -4897,7 +4897,7 @@ static inline bool netif_is_bond_dev(const struct net_device *dev)
...
> }
>@@ -4964,7 +4964,7 @@ static inline bool netif_is_lag_master(const struct net_device *dev)
> 
...
>+/* end legacy sysfs interface names */
	...
	Same comment about the "legacy sysfs" nomenclature as above.
...
>+	netdev_err(bond_dev, "(port %s): " fmt, (port_dev)->name, ##__VA_ARGS__)
	...
	Is the text format of log messages considered to be part of the
"protected" user API?  I personally don't think so, but I'm also
reasonably sure that there are applications that scrape the log and may
be perturbed by the change of wording here.
